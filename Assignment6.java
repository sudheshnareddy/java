 ...
 * not both with trailing zeroes, where v contains precisely all the digits from
 * x and from y,
...
class VampireClass {
    ...
    private boolean isValidFangs(long original, long fang1, long fang2) {
        ...
        if (hasTrailingZeros(fang1AsString) && hasTrailingZeros(fang2AsString))
            return false;

        int originalLength = numDigits(original);
        if (numDigits(fang1) != originalLength / 2 || numDigits(fang2) != originalLength / 2)
            return false;
